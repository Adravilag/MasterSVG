    ...
    if (isSprite) {
      ...
      try {
        ...
        if (fs.existsSync(cssPath)) {
          ...
          css = `/* Auto-generated by MasterSVG */\n.icon { --icon-size: 1em; display:inline-block; width:var(--icon-size); height:var(--icon-size); vertical-align:middle; background-color: currentColor; -webkit-mask-size: contain; mask-size: contain; -webkit-mask-repeat: no-repeat; mask-repeat: no-repeat; -webkit-mask-position: center; mask-position: center; }\n\n`;
        }

        const safeName = iconName.replace(/[^a-z0-9_-]/gi, '-').toLowerCase();
        const className = `ms-icon--${safeName}`;
        const rule = `.${className} { -webkit-mask-image: url("./${iconName}.svg"); mask-image: url("./${iconName}.svg"); }\n`;

        if (!css.includes(`.${className} `) && !css.includes(`.${className}{`)) {
          css += rule;
