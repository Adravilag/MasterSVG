...
export function registerReferenceCommands(
  ...
  const revealInTreeCmd = vscode.commands.registerCommand('iconManager.revealInTree', async (iconName: string, filePath?: string, lineNumber?: number) => {
    ...
    console.log(`[Icon Studio] revealInTree: name="${iconName}", path="${filePath}", line=${lineNumber}`);
    ...
    try {
      ...
      console.log(`[Icon Studio] findItemByIconNameOrPath:`, item?.label, item?.icon?.name, item?.icon?.path);
      
      if (!item) {
        // If not cached, try to find the icon and create an item
        let icon = workspaceSvgProvider.getIconByName(iconName);
        console.log(`[Icon Studio] getIconByName:`, icon?.name, icon?.path);
      ...
      if (item && workspaceTreeView) {
        try {
          ...
          console.log('[Icon Studio] Reveal failed, item may not be visible:', revealError);
        }
      }
    } catch (error) {
      // Silently fail - this is a nice-to-have feature
      console.log('[Icon Studio] Could not reveal in tree:', error);
    }
  });
