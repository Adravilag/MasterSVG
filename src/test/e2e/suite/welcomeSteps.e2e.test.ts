...
suite('Welcome Panel Steps State Tests', () => {
  ...
  function getStepNumberClasses(config: {
    ...
    const isWebComponentConfigured = !!(
      config.webComponentName && config.webComponentName.includes('-')
    );
  ...
  suite('Step 1 - Source Directory Configuration', () => {
    ...
    test('Step 1 has default folders configured in package.json', async () => {
      ...
      assert.strictEqual(
        hasDefaultFolders,
        true,
        'svgFolders has default values from package.json'
      );
    });
  ...
  suite('Step 3 - Build Format Configuration', () => {
    ...
    test('Step 3 completion requires buildFormat to be selected', async () => {
      ...
      assert.strictEqual(
        classesWithFormat.step3NumberClass,
        'completed',
  ...
  suite('Step 4 - Web Component Name Configuration', () => {
    ...
    test('Step 4 should NOT be completed when name has no hyphen', async () => {
      ...
      assert.strictEqual(
        ...
        false,
        'Step 4 should NOT be completed without hyphen'
      );
    });
    ...
    test('Step 4 should be completed when name has hyphen', async () => {
      ...
      assert.strictEqual(
        !!isWebComponentConfigured,
        true,
        'Step 4 should be completed with hyphen'
      );
    });
    ...
    test('Step 4 should be completed when sg-icon is explicitly set', async () => {
      ...
      assert.strictEqual(
        !!isWebComponentConfigured,
        true,
        'Step 4 should be completed when sg-icon is explicitly set'
      );
    });
    ...
    test('Step 4 should NOT be completed when webComponentName is empty (no default)', async () => {
      ...
      assert.strictEqual(
        !!isWebComponentConfigured,
        false,
        'Step 4 should NOT be completed when empty'
      );
    });
  });
  ...
  suite('Complete Button Enablement Logic', () => {
    ...
    test('Button disabled logic depends only on outputDirectory (since svgFolders has defaults)', async () => {
      ...
      assert.strictEqual(
        isOutputConfigured,
        false,
        'Button should be disabled when outputDirectory is empty'
      );
    });
    ...
    test('Button should still be DISABLED when only Step 1 AND Step 2 are configured', async () => {
      ...
      const outputDir = vscode.workspace
        .getConfiguration('iconStudio')
        .get<string>('outputDirectory', '');
      const buildFormat = vscode.workspace
        .getConfiguration('iconStudio')
    ...
    test('Button requires ALL 4 steps to be enabled', async () => {
      ...
      assert.strictEqual(isFullyConfigured, true, 'Button requires all 4 steps complete');
    });
  });
  ...
  suite('Full Configuration Flow', () => {
    test('Complete 4-step configuration flow', async () => {
      ...
      await delay(200);
      let svgFolders = vscode.workspace
        .getConfiguration('iconStudio')
        .get<string[]>('svgFolders', []);
      const step1Complete = svgFolders.length > 0 && !!svgFolders[0];
      ...
      await delay(200);
      let webComponentName = vscode.workspace
        .getConfiguration('iconStudio')
        .get<string>('webComponentName', '');
      const step4Complete = webComponentName && webComponentName.includes('-');
      assert.strictEqual(!!step4Complete, true, 'Step 4 should be complete');

      // Verify all 4 steps complete for button enabled
      svgFolders = vscode.workspace.getConfiguration('iconStudio').get<string[]>('svgFolders', []);
      outputDir = vscode.workspace
        .getConfiguration('iconStudio')
        .get<string>('outputDirectory', '');
      buildFormat = vscode.workspace.getConfiguration('iconStudio').get<string>('buildFormat', '');
      webComponentName = vscode.workspace
        .getConfiguration('iconStudio')
        .get<string>('webComponentName', '');
      ...
      assert.strictEqual(
        !!isFullyConfigured,
        true,
        'Button should be enabled after full 4-step configuration'
      );
    });
  });
  ...
  suite('Step Number CSS Classes (Green Check Marks)', () => {
    ...
    test('Step 2 should be DISABLED until Step 1 is complete', async () => {
      ...
      assert.strictEqual(
        classesStep1Incomplete.step2Disabled,
        true,
      ...
      assert.strictEqual(
        classesStep1Complete.step2Disabled,
        false,
    ...
    test('Step 3 should be DISABLED until Step 2 is complete', async () => {
      ...
      assert.strictEqual(
        classesStep2Incomplete.step3Disabled,
        true,
      ...
      assert.strictEqual(
        classesStep2Complete.step3Disabled,
        false,
    ...
    test('Step 4 should be DISABLED until Step 3 is complete (buildFormat selected)', async () => {
      ...
      assert.strictEqual(
        classesStep3Incomplete.step4Disabled,
        true,
      ...
      assert.strictEqual(
        classesStep3Complete.step4Disabled,
        false,
