...
suite('SVG Icons E2E Tests', () => {
  ...
  suite('Configuration with SVG Folders', () => {
    ...
    test('Should configure svgFolders with test path', async () => {
      ...
      const folders = vscode.workspace.getConfiguration('iconManager').get<string[]>('svgFolders', []);
    ...
    test('Should configure output directory', async () => {
      ...
      const configuredOutput = vscode.workspace.getConfiguration('iconManager').get<string>('outputDirectory', '');
    ...
    test('Should configure web component name', async () => {
      ...
      const name = vscode.workspace.getConfiguration('iconManager').get<string>('webComponentName', '');
  ...
  suite('Commands Registration', () => {
    ...
    test('buildIcons command should be registered', async () => {
      ...
      assert.ok(commands.includes('iconManager.buildIcons'), 'buildIcons command should exist');
    });
    
    test('refreshIcons command should be registered', async () => {
      const commands = await vscode.commands.getCommands(true);
      assert.ok(commands.includes('iconManager.refreshIcons'), 'refreshIcons command should exist');
    });
    
    test('refreshFiles command should be registered', async () => {
      const commands = await vscode.commands.getCommands(true);
      assert.ok(commands.includes('iconManager.refreshFiles'), 'refreshFiles command should exist');
    });
    
    test('generateSprite command should be registered', async () => {
      const commands = await vscode.commands.getCommands(true);
      assert.ok(commands.includes('iconManager.generateSprite'), 'generateSprite command should exist');
    });
    
    test('transformSvg command should be registered', async () => {
      const commands = await vscode.commands.getCommands(true);
      assert.ok(commands.includes('iconManager.transformSvg'), 'transformSvg command should exist');
    });
    
    test('optimizeSvg command should be registered', async () => {
      const commands = await vscode.commands.getCommands(true);
      assert.ok(commands.includes('iconManager.optimizeSvg'), 'optimizeSvg command should exist');
    });
    
    test('searchIcons command should be registered', async () => {
      const commands = await vscode.commands.getCommands(true);
      assert.ok(commands.includes('iconManager.searchIcons'), 'searchIcons command should exist');
    });
  });
  ...
  suite('Refresh Commands', () => {
    ...
    test('refreshFiles should execute without errors', async () => {
      try {
        await vscode.commands.executeCommand('iconManager.refreshFiles');
        await delay(500);
    ...
    test('refreshCode should execute without errors', async () => {
      try {
        await vscode.commands.executeCommand('iconManager.refreshCode');
        await delay(500);
    ...
    test('refreshBuilt should execute without errors', async () => {
      try {
        await vscode.commands.executeCommand('iconManager.refreshBuilt');
        await delay(500);
  ...
  suite('Build Format Configuration', () => {
    ...
    test('Should set build format to icons.ts', async () => {
      ...
      const format = vscode.workspace.getConfiguration('iconManager').get<string>('buildFormat', '');
  ...
  suite('Tree View Commands', () => {
    ...
    test('expandAll command should exist', async () => {
      const commands = await vscode.commands.getCommands(true);
      assert.ok(commands.includes('iconManager.expandAll'), 'expandAll should be registered');
    });
    
    test('collapseAll command should exist', async () => {
      const commands = await vscode.commands.getCommands(true);
      assert.ok(commands.includes('iconManager.collapseAll'), 'collapseAll should be registered');
    });
    
    test('expandSvgFiles command should exist', async () => {
      const commands = await vscode.commands.getCommands(true);
      assert.ok(commands.includes('iconManager.expandSvgFiles'), 'expandSvgFiles should be registered');
    });
    
    test('collapseSvgFiles command should exist', async () => {
      const commands = await vscode.commands.getCommands(true);
      assert.ok(commands.includes('iconManager.collapseSvgFiles'), 'collapseSvgFiles should be registered');
    });
  });
  ...
  suite('Panel Commands', () => {
    ...
    test('openWelcome command should open panel', async () => {
      try {
        await vscode.commands.executeCommand('iconManager.openWelcome');
        await delay(500);
    ...
    test('openPanel command should exist', async () => {
      const commands = await vscode.commands.getCommands(true);
      assert.ok(commands.includes('iconManager.openPanel'), 'openPanel should be registered');
    });
  });
});
