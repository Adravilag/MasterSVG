...
suite('WelcomePanel E2E Tests', () => {
  ...
  test('Welcome command should be registered', async () => {
    ...
    assert.ok(
      commands.includes('iconManager.openWelcome'),
      'Welcome command should be registered'
    );
  });
  ...
  test('Configuration commands should work', async () => {
    ...
    const updatedConfig = vscode.workspace.getConfiguration('iconManager');
  ...
  test('Web component name validation', async () => {
    ...
    await new Promise(resolve => setTimeout(resolve, 200));
    let name = vscode.workspace.getConfiguration('iconManager').get<string>('webComponentName', '');
    ...
    await new Promise(resolve => setTimeout(resolve, 200));
    name = vscode.workspace.getConfiguration('iconManager').get<string>('webComponentName', '');
    assert.strictEqual(name, 'custom-svg-icon', 'Valid web component name with multiple hyphens should be set');
  ...
  test('All main commands should be registered', async () => {
    ...
    const expectedCommands = [
      'iconManager.openWelcome',
      'iconManager.buildIcons',
      'iconManager.refreshIcons',
      'iconManager.searchIcons'
    ];
