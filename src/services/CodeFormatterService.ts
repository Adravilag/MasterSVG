...
export class CodeFormatterService {
  ...
  highlightCode(svg: string): string {
    ...
    const codeLines = lines
      .map(line => {
        ...
        let highlighted = line.replace(/&/g, '⟨AMP⟩').replace(/</g, '⟨LT⟩').replace(/>/g, '⟨GT⟩');
  ...
  placeholdersToHtml(text: string): string {
    return text
      ...
      .replace(/⟨GT⟩/g, '&gt;')
      .replace(/⦃b⦄/g, '<b>')
      .replace(/⦃\/b⦄/g, '</b>') // Brackets
      .replace(/⦃t⦄/g, '<i>')
      .replace(/⦃\/t⦄/g, '</i>') // Tags
      .replace(/⦃a⦄/g, '<u>')
      .replace(/⦃\/a⦄/g, '</u>') // Attributes/Props
      .replace(/⦃s⦄/g, '<em>')
      .replace(/⦃\/s⦄/g, '</em>') // Strings
      .replace(/⦃c⦄/g, '<cite>')
      .replace(/⦃\/c⦄/g, '</cite>') // Comments
      .replace(/⦃k⦄/g, '<kbd>')
      .replace(/⦃\/k⦄/g, '</kbd>') // CSS Keywords
      .replace(/⦃sel⦄/g, '<var>')
      .replace(/⦃\/sel⦄/g, '</var>') // Selectors
      .replace(/⦃v⦄/g, '<samp>')
      .replace(/⦃\/v⦄/g, '</samp>') // CSS Values
      .replace(/⦃br⦄/g, '<s>')
      .replace(/⦃\/br⦄/g, '</s>'); // Braces
  }
  ...
  getKeyframesForAnimation(animationType: string): string {
    ...
    return (
      keyframesMap[animationType] || `@keyframes ${animationType} {\n  /* Custom animation */\n}`
    );
  }
