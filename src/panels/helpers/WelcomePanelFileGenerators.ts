...
`;
  ...
  // Also create a minimal types.d.ts for editor integration
  try {
    generateCssTypes(outputPath);
  } catch (e) {
    // ignore errors
  }
}
...
/**
 * Generates a minimal `types.d.ts` file for CSS icon output so editors can provide basic types.
 */

/**
 * Generates a minimal `types.d.ts` file for CSS icon output so editors can provide basic types.
 */
function generateCssTypes(outputPath: string): void {
  const typesPath = path.join(outputPath, 'types.d.ts');
  const typesContent = `// Auto-generated by MasterSVG
// Types for CSS icon output
...
export function getExpectedOutputFiles(
  ...
  if (buildFormat === 'sprite.svg') {
    ...
    // Unified types filename for sprite output
