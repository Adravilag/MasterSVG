...
`;
  ...
  /**
   * Generate empty icons module files (svg-data.js, svg-element.js, types.d.ts)
   */
  private async _generateEmptyIconsModule(
    outputPath: string,
    webComponentName: string
  ): Promise<void> {
    this._generateEmptySvgDataFile(outputPath);
    this._generateIndexFile(outputPath);
    ...
    this._generateTypesDefinitions(outputPath, webComponentName);
  }

  private _generateEmptySvgDataFile(outputPath: string): void {
    const dataPath = path.join(outputPath, 'svg-data.js');
...
`;
  ...
  private _generateIndexFile(outputPath: string): void {
...
`;
  ...
  private _generateTypesDefinitions(outputPath: string, webComponentName: string): void {
